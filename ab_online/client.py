...
class Client:
    ...
    def generate_parser(self):
        parser = Parser(prog="ab-online",description="Launch reproducible Activity Browser sessions and distribute them using NoVNC.")
        ...
        parser.add_argument("--storage", nargs="?",
                            help="path of storage folder")
        parser.add_argument("--dev", action="store_true",
                            help="use local code instead of conda package")
        
        start = subparsers.add_parser("start",
                                      description="Start session(s).",
                                      help="start session(s)")
        ...
        build = subparsers.add_parser("build",
                                      description="Build session(s).",
                                      help="build session(s)")
        ...
        stop.add_argument("-r","--reset", action="store_true",
                          help="delete session containers storage")
        stop.add_argument("sessions", nargs="*", default="", 
                          help="session(s) name")
        ...
        setup.add_argument("session_file", nargs="+", default="", 
                          help="a session file")
        return parser
    ...
    def run(self, args, debug=True):
        ...
        if storage: config.STORAGE = True
        if dev: config.DEV = True
