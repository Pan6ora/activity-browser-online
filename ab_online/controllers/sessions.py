...
class Sessions():
    ...
    @classmethod
    def start_session(cls, session, restart=False, force_build=False, reset_storage=False):
    ...
    def create_storage(cls, session, reset_storage=False):
        ...
        for db in session.databases.values():
            Storage.add_file(f"{config.STORAGE}/databases/{db.filename}",
                            db.filename,
    ...
    def generate_tokens_file(cls, session):
        ...
        with open(f"{cls.storage_path}/{session.esc_name}/novnc/token.list", "w") as file:
            for i in range(session.machines):
    ...
    def generate_session_home(cls, session):
        ...
        with open(f"{cls.storage_path}/{session.esc_name}/home/index.html", "w") as file:
            for i in range(session.machines):
                file.write(f"<a href=\"https://{session.esc_name}.{config.DOMAIN}/vnc.html?resize=remote&amp;path=novnc/websockify?token={i}\">Machine n°{i}</a><br>\n")
        Storage.add_file(f"{config.INCLUDES}/httpd.conf","httpd.conf",f"sessions_storage/{session.esc_name}/home")
    ...
    def generate_main_home(cls):
        ...
        with open(f"{config.STORAGE}/proxy/static/index.html", "w") as file:
            for session in cls.running_sessions:
                file.write(f"<a href=\"https://home.{session.esc_name}.{config.DOMAIN}\">{session.name}</a><br>\n")

    @classmethod
    def generate_caddyfile(cls):
        with open(f"{config.STORAGE}/proxy/Caddyfile", "w") as file:
            file.writelines([f"{config.DOMAIN} \u007b \n",
                        "root * /home/home  \n",
                        "file_server browse \n",
                        "}                 \n"])
            for session in cls.running_sessions:
                file.writelines([f"{session.esc_name}.{config.DOMAIN} \u007b\n",
                        f"reverse_proxy {session.esc_name}-gate:8080\n",
                        "}\n"])
                file.writelines([f"home.{session.esc_name}.{config.DOMAIN} \u007b\n",
